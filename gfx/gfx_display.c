...
void gfx_display_draw_cursor_2(
      gfx_display_t *p_disp,
      void *userdata,
      unsigned video_width,
      unsigned video_height,
      bool cursor_visible,
      float *color, float cursor_size_x,float cursor_size_y,uintptr_t texture,
      float x, float y, unsigned width, unsigned height)
{
   gfx_display_ctx_draw_t draw;
   struct video_coords coords;
   ...
   draw.x               = x - (cursor_size_x / 2);
   draw.y               = (int)height - y - (cursor_size_y / 2);
   draw.width           = cursor_size_x;
   draw.height          = cursor_size_y;
   draw.coords          = &coords;
   draw.matrix_data     = NULL;
   draw.texture         = texture;
   draw.prim_type       = GFX_DISPLAY_PRIM_TRIANGLESTRIP;
